...
var user=txt.text()
cy.writeFile('cypress/fixtures/module1/userdata.txt',user +"\n")

})
cy.get('p[class="oxd-text oxd-text--p"]').last().then((txt)=>{
    var password=txt.text()
    cy.writeFile('cypress/fixtures/module1/userdata.txt',password,{flag:'a+'})
    })
cy.get("p[class='oxd-text oxd-text--p orangehrm-copyright']").first().then((text)=>{
    var version=text.text()
    cy.writeFile('cypress/fixtures/version.json',{"versionNumber" : version})
})
